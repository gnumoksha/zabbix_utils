...
function ajuda {
        echo "
Script para retornar o ip público na internet usando wget.
Versão: ${VERSAO}
...
function obtemIP {
	retorno=$($WGET -O - -q ${siteObtemIP} 2>/dev/null)
	debugar "wget retornou o codigo \"$?\""
...
if [[ -z $WGET || ! -x $WGET ]]; then
        debugar "Erro: software wget nao encontrado ou nao eh executavel"
        echo $NAO_SUPORTADO
        exit 1
fi
